package study.ywork.doc.itext.position;

import com.itextpdf.kernel.colors.DeviceGray;
import com.itextpdf.kernel.geom.PageSize;
import com.itextpdf.kernel.pdf.PdfDocument;
import com.itextpdf.kernel.pdf.PdfPage;
import com.itextpdf.kernel.pdf.PdfWriter;
import com.itextpdf.kernel.pdf.canvas.PdfCanvas;

import java.io.FileNotFoundException;

public class MovieTimeTable {
    protected static final int LOCATION_COUNT = 9;
    protected static final int TIMESLOTS = 32;
    protected static final float OFFSET_LEFT = 76;
    protected static final float WIDTH = 740;
    protected static final float OFFSET_BOTTOM = 36;
    protected static final float HEIGHT = 504;
    protected static final float OFFSET_LOCATION = 26;
    protected static final float WIDTH_LOCATION = 48;
    protected static final float HEIGHT_LOCATION = HEIGHT / LOCATION_COUNT;
    protected static final float WIDTH_TIMESLOT = WIDTH / TIMESLOTS;
    private static final String DEST = "movieTimeTable.pdf";

    public static void main(String[] args) throws Exception {
        new MovieTimeTable().manipulatePdf(DEST);
    }

    public void manipulatePdf(String dest) throws FileNotFoundException {
        PdfDocument pdfDoc = new PdfDocument(new PdfWriter(dest));
        pdfDoc.setDefaultPageSize(PageSize.A4.rotate());
        PdfPage page = pdfDoc.addNewPage();
        drawTimeTable(new PdfCanvas(page.newContentStreamBefore(), page.getResources(), pdfDoc));
        drawTimeSlots(new PdfCanvas(page.newContentStreamAfter(), page.getResources(), pdfDoc));
        pdfDoc.close();
    }

    protected void drawTimeTable(PdfCanvas canvas) {
        canvas.saveState().setLineWidth(1.2f);
        float llx = OFFSET_LEFT;
        float lly = OFFSET_BOTTOM;
        float urx = OFFSET_LEFT + WIDTH;
        float ury = OFFSET_BOTTOM + HEIGHT;
        canvas.moveTo(llx, lly).
            lineTo(urx, lly).
            lineTo(urx, ury).
            lineTo(llx, ury).
            closePath().
            stroke();

        llx = OFFSET_LOCATION;
        urx = OFFSET_LOCATION + WIDTH_LOCATION;
        ury = OFFSET_BOTTOM + HEIGHT;
        canvas.moveTo(llx, lly).
            lineTo(urx, lly).
            lineTo(urx, ury).
            lineTo(llx, ury).
            closePathStroke();

        canvas.setLineWidth(1).
            moveTo(OFFSET_LOCATION + WIDTH_LOCATION / 2, OFFSET_BOTTOM).
            lineTo(OFFSET_LOCATION + WIDTH_LOCATION / 2, OFFSET_BOTTOM + HEIGHT);

        for (int i = 1; i < LOCATION_COUNT; i++) {
            float y = OFFSET_BOTTOM + (i * HEIGHT_LOCATION);
            if (i == 2 || i == 6) {
                canvas.moveTo(OFFSET_LOCATION, y);
                canvas.lineTo(OFFSET_LOCATION + WIDTH_LOCATION, y);
            } else {
                canvas.moveTo(OFFSET_LOCATION + WIDTH_LOCATION / 2, y);
                canvas.lineTo(OFFSET_LOCATION + WIDTH_LOCATION, y);
            }
            canvas.moveTo(OFFSET_LEFT, y);
            canvas.lineTo(OFFSET_LEFT + WIDTH, y);
        }

        canvas.stroke();
        canvas.restoreState();
    }

    protected void drawTimeSlots(PdfCanvas canvas) {
        canvas.saveState();
        for (int i = 1; i < TIMESLOTS; i++) {
            float x = OFFSET_LEFT + (i * WIDTH_TIMESLOT);
            canvas.moveTo(x, OFFSET_BOTTOM);
            canvas.lineTo(x, OFFSET_BOTTOM + HEIGHT);
        }

        canvas.setLineWidth(0.3f);
        canvas.setStrokeColor(DeviceGray.GRAY);
        canvas.setLineDash(3, 1);
        canvas.stroke();
        canvas.restoreState();
    }
}
