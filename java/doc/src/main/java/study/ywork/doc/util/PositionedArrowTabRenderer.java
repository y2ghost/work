...
public class PositionedArrowTabRenderer extends TabRenderer {
    ...
    public PositionedArrowTabRenderer(Tab tab, Document doc, boolean isLeft) {
        ...
        try {
            font = PdfFontFactory.createFont(StandardFonts.ZAPFDINGBATS, PdfEncodings.WINANSI);
    ...
    public void draw(DrawContext drawContext) {
        ...
        try (Canvas canvas = new Canvas(drawContext.getCanvas(), rect)) {
            if (isLeft) {
                canvas.showTextAligned(p, pageSize.getLeft() - 15, rect.getBottom() + 5, TextAlignment.CENTER, VerticalAlignment.MIDDLE);
            } else {
                canvas.showTextAligned(p, pageSize.getRight() + 15, rect.getBottom() + 5, drawContext.getDocument().getNumberOfPages(),
                        TextAlignment.CENTER, VerticalAlignment.MIDDLE, (float) Math.PI);
            }
        }
    }
}
