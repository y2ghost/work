...
public class Layers {
    private static final String DEST = "layers.pdf";
    private static final String SOURCE = "layers_orig.pdf";
    private static final String RESOURCE = "src/main/resources/images/loa.jpg";

    public static void main(String[] args) throws IOException {
        new Layers().manipulatePdf(DEST);
    }
    ...
    public void manipulatePdf(String dest) throws IOException {
        ...
        try (PdfDocument srcDoc = new PdfDocument(new PdfReader(SOURCE));
            ...
            for (int i = 1; i <= srcDoc.getNumberOfPages(); i++) {
                ...
                canvas.addXObjectWithTransformationMatrix(layer, 1f, 0, 0.4f, 0.4f, 72, 50F * i);
                canvas.beginText();
                canvas.setFontAndSize(font, 20);
                canvas.moveText(496D, 150D + 50 * i);
    ...
    public void createOriginalPdf(String filename) throws IOException {
        ...
        try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(filename));
            ...
            over.saveState();
            float sin = (float) Math.sin(Math.PI / 60);
            float cos = (float) Math.cos(Math.PI / 60);
            over.beginText();
            ...
            over.setTextMatrix(cos, sin, -sin, cos, 50, 324);
