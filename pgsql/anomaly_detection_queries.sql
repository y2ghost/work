...
COPY public.earthquakes FROM '/tmp/earthquakes1.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes2.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes3.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes4.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes5.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes6.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes7.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes8.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes9.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes10.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes11.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes12.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes13.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes14.csv' DELIMITER ',' CSV HEADER;
COPY public.earthquakes FROM '/tmp/earthquakes15.csv' DELIMITER ',' CSV HEADER;


SELECT mag, count(id) as earthquakes,
...
SELECT place, mag, ntile(100) over (partition by place order by mag) as ntile
FROM earthquakes
WHERE mag is not null
...
FROM (
    SELECT place, mag,ntile(4) over (partition by place order by mag) as ntile
    FROM earthquakes
    WHERE mag is not null
    and place = 'Central Alaska'
) a
...
SELECT percentile_cont(0.25) within group (order by mag) as pct_25,
...
SELECT percentile_cont(0.25) within group (order by mag) as pct_25_mag,
...
SELECT percentile_cont(0.25) within group (order by mag) as ntile_25,
       ...
       1.5 * (percentile_cont(0.75) within group (order by mag) -
           percentile_cont(0.25) within group (order by mag)) as iqr,
       percentile_cont(0.25) within group (order by mag) -
           (1.5 * (percentile_cont(0.75) within group (order by mag) 
               - percentile_cont(0.25) within group (order by mag))) as lower_whisker,
       percentile_cont(0.75) within group (order by mag) +
           (1.5 * (percentile_cont(0.75) within group (order by mag)
               - percentile_cont(0.25) within group (order by mag))) as upper_whisker
FROM earthquakes
WHERE place like '%Japan%';

SELECT date_part('year',time)::int as year,mag
FROM earthquakes
WHERE place like '%Japan%'
ORDER BY 1,2;
...
case when place like '% of %' then split_part(place,' of ',2) 
     else place end as place_name,count(*)
FROM earthquakes
WHERE depth > 600
GROUP BY 1
ORDER BY 2 desc;
...
SELECT date_trunc('month',time)::date as earthquake_month,status,
       count(*) as earthquakes
FROM earthquakes
GROUP BY 1,2
ORDER BY 1;
...
case when place like '% of %' then split_part(place,' of ',2)
     ...
     end as place,count(*) as earthquakes
FROM earthquakes
WHERE mag >= 6
GROUP BY 1
ORDER BY 2 desc;

SELECT place,extract('days' from '2020-12-31 23:59:59' - latest) as days_since_latest,
       count(*) as earthquakes,
    ...
    FROM (
        SELECT 
            replace(initcap(
                ...
                ),'Region','') as place, time
        FROM earthquakes WHERE mag > 5
    ) a
) a         
GROUP BY 1,2;
