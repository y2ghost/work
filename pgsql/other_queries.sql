...
SELECT date_part('year',interval_since_last) as years_since_last,
       count(*) as reps
FROM (
    SELECT id_bioguide, max(term_start) as max_date,
...
FROM (
    SELECT date_part('year',interval_since_last) * 12 
        + date_part('year',interval_since_last)
        as months_since_last,count(*) as reps
    FROM (
        SELECT id_bioguide,
...
SELECT product1, product2, count(customer_id) as customers
FROM (
    SELECT a.customer_id, a.product as product1, b.product as product2
    FROM purchases a
    JOIN purchases b
    ON a.customer_id = b.customer_id and b.product > a.product
) a
GROUP BY 1,2
ORDER BY 3 desc;
