...
COPY videogame_sales FROM '/tmp/videogame_sales.csv' DELIMITER ',' CSV HEADER;


SELECT type, mag, case when place like '%CA%' then 'California'
...
SELECT date_part('year',c.first_term) as first_year,a.party,
...
SELECT case when state in ('CA','TX','FL','NY','PA') then state 
    else 'Other' end as state_group,count(*) as terms
FROM legislators_terms
GROUP BY 1
ORDER BY 2 desc;
...
JOIN (
    SELECT state, count(distinct id_bioguide),
