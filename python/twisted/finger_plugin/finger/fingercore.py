...
class UserStatusTree(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        def cbUsers(users):
            usersInfo = self.template % {'users': ''.join([
                    '<li><a href="%s">%s</a></li>' % (name, name)
                    for name in users])}
            request.write(bytes(usersInfo, encoding="utf8"))
            request.finish()
...
components.registerAdapter(UserStatusTree,
    IFingerService,
    resource.IResource)
...
class UserStatus(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        def cbStatus(status):
            userInfo = self.template % {
                'title': self.user,
                'name': self.user,
                'status': status}
            request.write(bytes(userInfo, encoding='utf8'))
            request.finish()
